                                           ...
                                           uint16_t size)
{
    ...
    case SDP_EVENT_QUERY_COMPLETE:
        ...
        {
            ESP_LOGI(TAG, "SDP query failed 0x%02x", sdp_event_query_complete_get_status(packet));
            break;
        }
...
uint8_t get_next_sequence_id()
{
    ...
    {
        ++seq_id;
    } while (seq_id == START_OF_FRAME || seq_id == END_OF_FRAME || seq_id == ESCAPE);
...
void handle_alexa_packet()
{
    ...
    ESP_LOGI(TAG, "Alexa Gadgets Toolkit packet, checksum: %02x %02x", (checksum >> 8) & 0xff, checksum & 0xff);
